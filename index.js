	...
	MongoClient.connect(url, function(err, db)       
		{                                        
			if (err)                          
				throw err;            
			let dbo=db.db("WT");     
			dbo.collection("rbankt").findOne({mail:req.body.from.mail},(err,result)=>
				{
					if(err)
						throw err;
					else
	...
	MongoClient.connect(url, function(err, db)
		{
			if (err)
				throw err;
			let dbo=db.db("WT");
			dbo.collection("rbankc").find({}).toArray((err,result)=>             
				{                         
					// console.log(result);
					res.send(JSON.stringify(result));
				});             
...
//	console.log(req.body);
	...
	MongoClient.connect(url, function(err, db) {    
		...
		let bal1,bal2;
		dbo.collection("rbankc").findOne({mail:req.body.from.mail},(err,result)=>{
			if(err)
				throw err;
			else
			{
		...
		dbo.collection("rbankc").findOne({mail:req.body.to.mail},(err,result)=>{            
			if(err)                           
				throw(err);                      
			else     
			{
		...
		function after()
		{
			...
			dbo.collection("rbankt").updateOne(query,update,(err,result)=>{                                                                if(err)                                                            throw(err);
			...
			dbo.collection("rbankc").updateOne(query,update,(err,result)=>{                                                                if(err)                                                            throw(err);
				//else                                                               console.log(result);                   
			});
